...
function loadPage(page) {
  switch(page) {
    ...
    case 'client-portal': loadClientPortal(); break;
  }
}
...
/* ═══════════════════════════════════════════════
   CLIENT PORTAL — Dummy account (1:1 replica of October AI client portal)
   ═══════════════════════════════════════════════ */

var cpState = {
  tab: 'agent',          // agent | analytics | conversations | leads | build | knowledge | settings
  analyticsPeriod: 30,   // 7 | 30 | 0 (all time)
  convoFilter: 'all',    // all | converted | not_converted
  convoPage: 1,
  convoPerPage: 20,
  buildSection: null,     // which build section to scroll to
  settingsSaved: false
};
...
var DUMMY_AGENT = {
  ...
  language: 'en',
  brandColor: '#1a1a1a',
  conversionUrl: 'https://booking.harbourviewhotel.com/reserve',
  isActive: true,
  minutesUsed: 342,
  minutesQuota: 1000,
  ...
  wordCount: 1847,
  propertyData: 'Harbour View Hotel is a premium boutique hotel located in central Copenhagen, overlooking the harbour. We offer 42 uniquely designed rooms across 4 floors, including Standard, Superior, and Suite categories.\n\nAmenities: Rooftop terrace bar, spa, fitness center, conference rooms, restaurant, 24/7 concierge.\n\nCheck-in: 15:00 | Check-out: 11:00\nAddress: Nyhavn 42, 1051 Copenhagen K\nPhone: +45 33 12 34 56\nEmail: info@harbourviewhotel.com\n\nRoom Types:\n- Standard Room: 22 sqm, city view, from DKK 1,295/night\n- Superior Room: 30 sqm, harbour view, from DKK 1,795/night\n- Junior Suite: 42 sqm, harbour view + balcony, from DKK 2,495/night\n- Penthouse Suite: 65 sqm, panoramic view + terrace, from DKK 4,995/night\n\nRestaurant: "The Harbour Kitchen" serves Nordic-inspired cuisine. Breakfast 07:00-10:30, Dinner 18:00-22:00.\n\nSpa: Open daily 08:00-20:00. Treatments include massage, facial, sauna, steam room.',
  demoQuestions: [
    'What rooms do you have available?',
    'Tell me about the restaurant',
    'How do I get to the hotel?',
    'What spa treatments do you offer?',
    'Can I see the harbour view rooms?'
  ],
  roomMappings: {
    lobby: { label: 'Hotel Lobby', sweepId: 'abc123def' },
    standard: { label: 'Standard Room', sweepId: 'ghi456jkl' },
    superior: { label: 'Superior Room', sweepId: 'mno789pqr' },
    suite: { label: 'Junior Suite', sweepId: 'stu012vwx' },
    restaurant: { label: 'The Harbour Kitchen', sweepId: 'yza345bcd' },
    spa: { label: 'Spa & Wellness', sweepId: 'efg678hij' },
    rooftop: { label: 'Rooftop Terrace', sweepId: 'klm901nop' }
  },
  embedCode: '<script>\n  window.OCTOBER_TENANT_ID = "demo-tenant-001";\n  window.OCTOBER_SERVER = "https://app.october-ai.com";\n</script>\n<script src="https://app.october-ai.com/kunde/embed.js"></script>'
};

/* Generate realistic dummy conversations */
function cpGenerateConversations() {
  var convos = [];
  var names = ['Emma Nielsen', 'Jonas Petersen', 'Sophie Andersen', 'Lars Hansen', 'Maria Christensen', 'Mikkel Jensen', 'Anna Larsen', 'Frederik Olsen', 'Ida Thomsen', 'Christian Rasmussen', 'Laura Mortensen', 'Andreas Madsen', 'Julie Frederiksen', 'Rasmus Eriksen', 'Camilla Holm', 'Thomas Berg', 'Cecilie Dahl', 'Mads Lind', 'Nanna Storm', 'Oliver Nygaard', 'Katrine Moller', 'Nikolaj Bak', 'Sofie Krogh', 'Jakob Skov', 'Emilie West', 'Martin Pihl', 'Sarah Voss', 'Alexander Falk', 'Mathilde Blom', 'Simon Hald'];
  var emails = names.map(function(n) { return n.toLowerCase().replace(/ /g, '.') + '@email.com'; });
  var phones = ['+45 20 12 34 56', '+45 31 23 45 67', '+45 42 34 56 78', '', '+45 53 45 67 89', '', '+45 64 56 78 90', '+45 75 67 89 01', '', '+45 86 78 90 12', '', '+45 97 89 01 23', '+45 28 90 12 34', '', '+45 39 01 23 45', '+45 40 12 34 56', '', '+45 51 23 45 67', '+45 62 34 56 78', '', '+45 73 45 67 89', '', '+45 84 56 78 90', '+45 95 67 89 01', '', '+45 26 78 90 12', '+45 37 89 01 23', '', '+45 48 90 12 34', '+45 59 01 23 45'];
  var spacesShown = [['lobby', 'superior'], ['standard'], ['lobby', 'suite', 'rooftop'], ['superior', 'restaurant'], ['lobby'], ['suite', 'spa'], ['standard', 'superior'], ['lobby', 'restaurant', 'spa'], ['rooftop'], ['superior', 'suite'], ['lobby', 'standard'], ['restaurant'], ['lobby', 'superior', 'restaurant'], ['spa', 'rooftop'], ['standard'], ['suite'], ['lobby', 'spa'], ['superior', 'rooftop'], ['lobby'], ['standard', 'restaurant'], ['lobby', 'superior'], ['suite', 'spa'], ['rooftop', 'restaurant'], ['standard'], ['lobby', 'suite'], ['superior'], ['spa'], ['lobby', 'standard', 'superior'], ['restaurant', 'rooftop'], ['lobby']];

  var sampleMessages = [
    [
      { role: 'assistant', content: 'Welcome to Harbour View Hotel! I\'d love to show you around our beautiful property overlooking Copenhagen harbour. What would you like to explore?' },
      { role: 'user', content: 'I\'m looking for a room with a harbour view' },
      { role: 'assistant', content: 'Great choice! Our Superior Rooms offer stunning harbour views. Let me show you one — they\'re 30 square metres with floor-to-ceiling windows. Shall I take you there?' },
      { role: 'user', content: 'Yes please' },
      { role: 'assistant', content: 'Here we are in our Superior Room. As you can see, the harbour view is absolutely breathtaking. Prices start from DKK 1,795 per night. Would you like to check availability?' },
      { role: 'user', content: 'What dates do you have available next weekend?' },
      { role: 'assistant', content: 'I\'d recommend booking through our reservation page for real-time availability. Let me open that for you!' }
    ],
    [
      { role: 'assistant', content: 'Hello! Welcome to Harbour View Hotel. How can I help you today?' },
      { role: 'user', content: 'What kind of rooms do you have?' },
      { role: 'assistant', content: 'We have four room categories: Standard Rooms from DKK 1,295, Superior Rooms with harbour view from DKK 1,795, Junior Suites with balcony from DKK 2,495, and our stunning Penthouse Suite from DKK 4,995. Which interests you?' },
      { role: 'user', content: 'The junior suite sounds nice. Can you show me?' },
      { role: 'assistant', content: 'Absolutely! Here\'s our Junior Suite — 42 sqm with a private balcony overlooking the harbour. It\'s one of our most popular choices for special occasions.' }
    ],
    [
      { role: 'assistant', content: 'Welcome! I\'m your virtual concierge at Harbour View Hotel. Shall I give you a tour?' },
      { role: 'user', content: 'Tell me about the restaurant' },
      { role: 'assistant', content: 'The Harbour Kitchen serves Nordic-inspired cuisine with fresh, locally sourced ingredients. Breakfast is served 07:00-10:30, and dinner 18:00-22:00. Let me show you the space!' },
      { role: 'user', content: 'That looks lovely. Do I need reservations for dinner?' },
      { role: 'assistant', content: 'For dinner, we recommend booking in advance, especially on weekends. Guests can reserve through our booking page or ask reception directly.' }
    ]
  ];

  for (var i = 0; i < 30; i++) {
    var dayOffset = Math.floor(Math.random() * 60);
    var date = new Date();
    date.setDate(date.getDate() - dayOffset);
    date.setHours(Math.floor(Math.random() * 14) + 8, Math.floor(Math.random() * 60));
    var converted = Math.random() < 0.18;
    var duration = Math.floor(Math.random() * 300) + 60;
    var msgIdx = i % sampleMessages.length;
    var msgCount = sampleMessages[msgIdx].length + Math.floor(Math.random() * 4);
    ...
    convos.push({
      ...
      spacesShown: spacesShown[i % spacesShown.length],
      converted: converted,
      messages: sampleMessages[msgIdx],
      lead: (Math.random() < 0.4) ? {
        name: names[i],
        email: emails[i],
        phone: phones[i] || null,
        date: date.toISOString()
  ...
  convos.sort(function(a, b) { return new Date(b.date) - new Date(a.date); });
  return convos;
}

var DUMMY_CONVERSATIONS = cpGenerateConversations();

function cpGetConversations(period) {
  var now = new Date();
  return DUMMY_CONVERSATIONS.filter(function(c) {
    if (period === 0) return true;
    var diff = (now - new Date(c.date)) / (1000 * 60 * 60 * 24);
    return diff <= period;
  });
}
...
function cpGetLeads() {
  var leads = [];
  DUMMY_CONVERSATIONS.forEach(function(c) {
    if (c.lead) {
      leads.push({
        name: c.lead.name,
        email: c.lead.email,
...
/* ── Main Client Portal Renderer ── */
function loadClientPortal() {
  var c = document.getElementById('page-client-portal');
  var tab = cpState.tab;
  ...
  var html = '<div class="page-label">CLIENT PORTAL</div>'
    + '<h1 class="page-heading">Dummy Account — ' + esc(DUMMY_AGENT.agentName) + '</h1>'
    + '<p class="page-sub">1:1 replica of the October AI client portal with mock data</p>'
    + '<div class="cp-nav">';

  navTabs.forEach(function(t) {
    html += '<button class="cp-nav-btn' + (tab === t.id ? ' active' : '') + '" onclick="cpState.tab=\'' + t.id + '\';loadClientPortal()">' + t.label + '</button>';
  });
  html += '</div><div class="cp-content">';

  switch (tab) {
    case 'agent': html += cpRenderAgent(); break;
    case 'analytics': html += cpRenderAnalytics(); break;
    case 'conversations': html += cpRenderConversations(); break;
    case 'leads': html += cpRenderLeads(); break;
    case 'build': html += cpRenderBuild(); break;
    case 'knowledge': html += cpRenderKnowledge(); break;
    case 'settings': html += cpRenderSettings(); break;
  }
  ...
  // Post-render hooks
  if (tab === 'analytics') cpRenderAnalyticsCharts();
  if (tab === 'knowledge') cpUpdateWordBar();
  if (tab === 'build') cpUpdateBuildWordBar();
}

/* ── Tab: Your Agent (Dashboard) ── */
function cpRenderAgent() {
  var convos = cpGetConversations(30);
  var converted = convos.filter(function(c) { return c.converted; }).length;
  var rate = convos.length > 0 ? Math.round((converted / convos.length) * 100) : 0;

  return '<div class="cp-agent-card">'
    + '<div class="cp-agent-header">'
    + '<div class="cp-agent-icon" style="background:' + esc(DUMMY_AGENT.brandColor) + '"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.5"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg></div>'
    + '<div class="cp-agent-info">'
    + '<h2>' + esc(DUMMY_AGENT.agentName) + '</h2>'
    + '<span class="badge badge-active">Active</span>'
    + '</div>'
    + '</div>'
    + '<div class="cp-agent-stats">'
    + '<div class="cp-stat"><div class="cp-stat-value">' + convos.length + '</div><div class="cp-stat-label">Conversations (30d)</div></div>'
    + '<div class="cp-stat"><div class="cp-stat-value">' + converted + '</div><div class="cp-stat-label">Conversions (30d)</div></div>'
    + '<div class="cp-stat"><div class="cp-stat-value">' + rate + '%</div><div class="cp-stat-label">Conversion Rate</div></div>'
    + '</div>'
    + '<div class="cp-agent-actions">'
    + '<button class="btn" onclick="cpState.tab=\'analytics\';loadClientPortal()">Analytics</button>'
    + '<button class="btn btn-outline" onclick="cpState.tab=\'build\';loadClientPortal()">Build</button>'
    + '<button class="btn btn-outline" onclick="cpState.tab=\'settings\';loadClientPortal()">Settings</button>'
    + '</div>'
    + '</div>';
}
...
function cpRenderAnalytics() {
  var period = cpState.analyticsPeriod;
  var convos = cpGetConversations(period);
  var converted = convos.filter(function(c) { return c.converted; }).length;
  var rate = convos.length > 0 ? ((converted / convos.length) * 100).toFixed(1) : '0.0';
  var totalDuration = convos.reduce(function(s, c) { return s + c.duration; }, 0);
  var avgDuration = convos.length > 0 ? Math.round(totalDuration / convos.length) : 0;
  var withSpaces = convos.filter(function(c) { return c.spacesShown.length > 0; }).length;

  // Popular questions (mock)
  var popularQuestions = [
    { q: 'What rooms do you have available?', count: 18 },
    { q: 'Tell me about the restaurant', count: 14 },
    { q: 'How much does a suite cost?', count: 11 },
    { q: 'What spa treatments do you offer?', count: 9 },
    { q: 'Can I see the harbour view?', count: 8 },
    { q: 'What time is check-in?', count: 7 },
    { q: 'Do you have parking?', count: 6 },
    { q: 'Is breakfast included?', count: 5 }
  ];

  // Space performance
  var spacePerf = {};
  convos.forEach(function(cv) {
    cv.spacesShown.forEach(function(sp) {
      if (!spacePerf[sp]) spacePerf[sp] = { shown: 0, converted: 0 };
      spacePerf[sp].shown++;
      if (cv.converted) spacePerf[sp].converted++;
    });
  });
  var spaceRows = Object.keys(spacePerf).map(function(k) {
    return { key: k, label: (DUMMY_AGENT.roomMappings[k] && DUMMY_AGENT.roomMappings[k].label) || k, shown: spacePerf[k].shown, converted: spacePerf[k].converted };
  }).sort(function(a, b) { return b.shown - a.shown; });

  // Funnel
  var totalVisitors = convos.length + Math.floor(convos.length * 0.4);
  var started = convos.length;
  var shownSpace = withSpaces;
  var clickedConversion = converted;

  var html = '<div class="cp-period-filter">'
    + '<button class="cp-period-btn' + (period === 7 ? ' active' : '') + '" onclick="cpState.analyticsPeriod=7;loadClientPortal()">7 days</button>'
    + '<button class="cp-period-btn' + (period === 30 ? ' active' : '') + '" onclick="cpState.analyticsPeriod=30;loadClientPortal()">30 days</button>'
    + '<button class="cp-period-btn' + (period === 0 ? ' active' : '') + '" onclick="cpState.analyticsPeriod=0;loadClientPortal()">All time</button>'
    + '</div>';

  // Metric cards
  html += '<div class="cp-metrics">'
    + cpMetricCard('Conversations', convos.length)
    + cpMetricCard('Conversions', converted)
    + cpMetricCard('Conversion Rate', rate + '%')
    + cpMetricCard('Avg Duration', fmtDuration(avgDuration))
    + '</div>';

  // Conversations chart
  html += '<div class="cp-section"><h3 class="cp-section-title">Conversations Over Time</h3><canvas id="cpChartConvos" height="200"></canvas></div>';

  // Conversion funnel
  html += '<div class="cp-section"><h3 class="cp-section-title">Conversion Funnel</h3>'
    + '<div class="cp-funnel">'
    + cpFunnelBar('Total visitors', totalVisitors, totalVisitors)
    + cpFunnelBar('Started conversation', started, totalVisitors)
    + cpFunnelBar('Shown a space', shownSpace, totalVisitors)
    + cpFunnelBar('Clicked conversion', clickedConversion, totalVisitors)
    + '</div></div>';

  // Popular questions
  html += '<div class="cp-section"><h3 class="cp-section-title">Popular Questions</h3><div class="cp-questions-list">';
  popularQuestions.forEach(function(pq) {
    html += '<div class="cp-question-row"><span class="cp-question-text">' + esc(pq.q) + '</span><span class="cp-question-count">' + pq.count + '</span></div>';
  });
  html += '</div></div>';

  // Space performance
  html += '<div class="cp-section"><h3 class="cp-section-title">Space Performance</h3>'
    + '<table class="data-table"><thead><tr><th>Space</th><th style="text-align:right">Times Shown</th><th style="text-align:right">Conversions</th></tr></thead><tbody>';
  spaceRows.forEach(function(sp) {
    html += '<tr><td>' + esc(sp.label) + '</td><td style="text-align:right">' + sp.shown + '</td><td style="text-align:right">' + sp.converted + '</td></tr>';
  });
  html += '</tbody></table></div>';

  // Action buttons
  html += '<div class="cp-action-row">'
    + '<button class="btn btn-outline" onclick="cpState.tab=\'conversations\';loadClientPortal()">View all conversations</button>'
    + '<button class="btn btn-outline" onclick="cpState.tab=\'leads\';loadClientPortal()">View leads</button>'
    + '</div>';

  return html;
}

function cpMetricCard(label, value) {
  return '<div class="cp-metric"><div class="cp-metric-value">' + value + '</div><div class="cp-metric-label">' + label + '</div></div>';
}

function cpFunnelBar(label, count, total) {
  var pct = total > 0 ? Math.round((count / total) * 100) : 0;
  var width = total > 0 ? Math.max(8, (count / total) * 100) : 8;
  return '<div class="cp-funnel-row">'
    + '<div class="cp-funnel-label">' + label + '</div>'
    + '<div class="cp-funnel-bar-wrap">'
    + '<div class="cp-funnel-bar" style="width:' + width + '%"></div>'
    + '</div>'
    + '<div class="cp-funnel-val">' + count + ' (' + pct + '%)</div>'
    + '</div>';
}

function cpRenderAnalyticsCharts() {
  var period = cpState.analyticsPeriod;
  var convos = cpGetConversations(period);
  var days = period === 0 ? 60 : period;
  ...
  convos.forEach(function(cv) {
    var key = new Date(cv.date).toISOString().split('T')[0];
    if (dayCounts[key] !== undefined) dayCounts[key]++;
  });

  var labels = Object.keys(dayCounts).reverse().map(function(d) {
    return new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
  });
  var data = Object.keys(dayCounts).reverse().map(function(k) { return dayCounts[k]; });

  renderChart('cpChartConvos', 'bar', labels, data, 'Conversations');
}

/* ── Tab: Conversations ── */
function cpRenderConversations() {
  var convos = cpGetConversations(30);
  var filter = cpState.convoFilter;

  if (filter === 'converted') convos = convos.filter(function(c) { return c.converted; });
  else if (filter === 'not_converted') convos = convos.filter(function(c) { return !c.converted; });

  var totalPages = Math.ceil(convos.length / cpState.convoPerPage);
  var pageConvos = convos.slice((cpState.convoPage - 1) * cpState.convoPerPage, cpState.convoPage * cpState.convoPerPage);

  var html = '<div class="cp-filter-bar">'
    + '<button class="cp-filter-btn' + (filter === 'all' ? ' active' : '') + '" onclick="cpState.convoFilter=\'all\';cpState.convoPage=1;loadClientPortal()">All</button>'
    + '<button class="cp-filter-btn' + (filter === 'converted' ? ' active' : '') + '" onclick="cpState.convoFilter=\'converted\';cpState.convoPage=1;loadClientPortal()">Converted</button>'
    + '<button class="cp-filter-btn' + (filter === 'not_converted' ? ' active' : '') + '" onclick="cpState.convoFilter=\'not_converted\';cpState.convoPage=1;loadClientPortal()">Not converted</button>'
    + '<button class="btn btn-outline btn-sm" style="margin-left:auto" onclick="cpExportCSV()">Export CSV</button>'
    + '</div>';

  html += '<table class="data-table"><thead><tr>'
    + '<th>Date</th><th>Duration</th><th>Messages</th><th>Spaces Shown</th><th>Converted</th>'
    + '</tr></thead><tbody>';

  pageConvos.forEach(function(cv) {
    var spaceLabels = cv.spacesShown.map(function(s) {
      return (DUMMY_AGENT.roomMappings[s] && DUMMY_AGENT.roomMappings[s].label) || s;
    }).join(', ');
    html += '<tr onclick="cpShowConversation(\'' + cv.id + '\')">'
      + '<td>' + new Date(cv.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) + '</td>'
      + '<td>' + fmtDuration(cv.duration) + '</td>'
      + '<td>' + cv.messageCount + '</td>'
      + '<td>' + esc(spaceLabels || '-') + '</td>'
      + '<td>' + (cv.converted ? '<span class="badge badge-active">Yes</span>' : '<span class="badge" style="background:#f5f5f5;color:#888">No</span>') + '</td>'
      + '</tr>';
  });
  ...
  // Pagination
  if (totalPages > 1) {
    html += '<div class="pagination"><span>Page ' + cpState.convoPage + ' of ' + totalPages + '</span><div class="pagination-btns">'
      + '<button class="page-btn" onclick="cpState.convoPage=Math.max(1,cpState.convoPage-1);loadClientPortal()"' + (cpState.convoPage <= 1 ? ' disabled' : '') + '>Prev</button>'
      + '<button class="page-btn" onclick="cpState.convoPage=Math.min(' + totalPages + ',cpState.convoPage+1);loadClientPortal()"' + (cpState.convoPage >= totalPages ? ' disabled' : '') + '>Next</button>'
      + '</div></div>';
  }

  // Conversation modal container
  html += '<div class="cp-convo-modal" id="cpConvoModal">'
    + '<div class="cp-convo-modal-backdrop" onclick="cpCloseConvoModal()"></div>'
    + '<div class="cp-convo-modal-card">'
    + '<button class="detail-close" onclick="cpCloseConvoModal()">&times;</button>'
    + '<div id="cpConvoModalContent"></div>'
    + '</div></div>';

  return html;
}

function cpShowConversation(id) {
  var cv = DUMMY_CONVERSATIONS.find(function(c) { return c.id === id; });
  if (!cv) return;

  var html = '<h3 class="cp-convo-modal-title">'
    + new Date(cv.date).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })
    + '</h3><div class="cp-convo-messages">';

  cv.messages.forEach(function(m) {
    html += '<div class="cp-msg cp-msg-' + m.role + '">'
      + '<div class="cp-msg-role">' + (m.role === 'user' ? 'Visitor' : 'Agent') + '</div>'
      + '<div class="cp-msg-text">' + esc(m.content) + '</div>'
      + '</div>';
  });
  html += '</div>';

  document.getElementById('cpConvoModalContent').innerHTML = html;
  document.getElementById('cpConvoModal').classList.add('open');
}

function cpCloseConvoModal() {
  var modal = document.getElementById('cpConvoModal');
  if (modal) modal.classList.remove('open');
}

function cpExportCSV() {
  showToast('CSV export triggered (demo mode)', 'success');
}

/* ── Tab: Leads ── */
function cpRenderLeads() {
  var leads = cpGetLeads();

  var html = '<div class="cp-filter-bar">'
    + '<span class="cp-lead-count">' + leads.length + ' leads collected</span>'
    + '<button class="btn btn-outline btn-sm" style="margin-left:auto" onclick="cpExportCSV()">Download CSV</button>'
    + '</div>';

  html += '<table class="data-table"><thead><tr>'
    + '<th>Name</th><th>Email</th><th>Phone</th><th>Date</th><th>Converted</th>'
    + '</tr></thead><tbody>';

  if (leads.length === 0) {
    html += '<tr><td colspan="5" style="text-align:center;padding:40px;color:var(--muted)">No leads collected yet</td></tr>';
  } else {
    leads.forEach(function(l) {
      html += '<tr>'
        ...
        + '<td>' + (l.converted ? '<span class="badge badge-active">Yes</span>' : '<span class="badge" style="background:#f5f5f5;color:#888">No</span>') + '</td>'
        + '</tr>';
    });
  }
  html += '</tbody></table>';
...
/* ── Tab: Build ── */
function cpRenderBuild() {
  var a = DUMMY_AGENT;
  var rmKeys = Object.keys(a.roomMappings);

  // Vertical selection
  var verticals = [
    { id: 'hotel', label: 'Hotel & Hospitality', icon: 'M3 21V7l9-4 9 4v14' },
    { id: 'real_estate_sale', label: 'Real Estate', icon: 'M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z' },
    { id: 'retail', label: 'Retail Showroom', icon: 'M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z' },
    { id: 'education', label: 'Education', icon: 'M22 10v6M2 10l10-5 10 5-10 5z' },
    { id: 'other', label: 'Other', icon: 'M12 2L2 7l10 5 10-5z' }
  ];

  var html = '';

  // Section: Vertical
  html += '<div class="cp-build-section">'
    + '<h3 class="cp-section-title">1. Select Vertical</h3>'
    + '<div class="cp-vertical-grid">';
  verticals.forEach(function(v) {
    html += '<div class="cp-vertical-card' + (a.vertical === v.id ? ' selected' : '') + '" onclick="DUMMY_AGENT.vertical=\'' + v.id + '\';loadClientPortal()">'
      + '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="28" height="28"><path d="' + v.icon + '"/></svg>'
      + '<span>' + v.label + '</span>'
      + '</div>';
  });
  html += '</div></div>';

  // Section: Space Mappings
  html += '<div class="cp-build-section">'
    + '<h3 class="cp-section-title">2. Space Mappings</h3>'
    + '<p class="cp-build-help">Map your Matterport tour spaces so the AI agent can navigate visitors to specific rooms.</p>'
    + '<div class="cp-spaces-list">';

  rmKeys.forEach(function(key) {
    var rm = a.roomMappings[key];
    html += '<div class="cp-space-row">'
      + '<div class="cp-space-dot connected"></div>'
      + '<div class="cp-space-info">'
      + '<div class="cp-space-label">' + esc(rm.label) + '</div>'
      + '<div class="cp-space-sweep">Sweep ID: ' + esc(rm.sweepId) + '</div>'
      + '</div>'
      + '<button class="btn-sm btn-ghost" onclick="showToast(\'Edit mode (demo)\')">Edit</button>'
      + '</div>';
  });
  html += '</div>'
    + '<button class="btn btn-outline btn-sm" onclick="showToast(\'Add space (demo)\', \'success\')" style="margin-top:12px">+ Add Space</button>'
    + '</div>';

  // Section: Your Agent's Data
  html += '<div class="cp-build-section">'
    + '<h3 class="cp-section-title">3. Your Agent\'s Data</h3>'
    + '<div class="cp-word-bar-wrap"><div class="cp-word-bar" id="cpBuildWordBar"></div></div>'
    + '<div class="cp-word-count" id="cpBuildWordCount"></div>'
    + '<div class="cp-data-tabs">'
    + '<button class="cp-data-tab active" onclick="cpSwitchBuildTab(\'sync\', this)">Sync from website</button>'
    + '<button class="cp-data-tab" onclick="cpSwitchBuildTab(\'upload\', this)">Upload file</button>'
    + '<button class="cp-data-tab" onclick="cpSwitchBuildTab(\'manual\', this)">Add manually</button>'
    + '</div>'
    + '<div class="cp-data-pane" id="cpBuildPane-sync" style="display:block">'
    + '<div class="cp-sync-row"><input class="search-input" placeholder="https://your-website.com" value="https://harbourviewhotel.com" style="flex:1"><button class="btn btn-sm" onclick="showToast(\'Scraping... (demo)\', \'success\')">Sync</button></div>'
    + '</div>'
    + '<div class="cp-data-pane" id="cpBuildPane-upload" style="display:none">'
    + '<div class="cp-dropzone" onclick="showToast(\'File picker (demo)\')"><div class="cp-dropzone-text">Drop a file here or click to upload<br><span style="color:var(--muted);font-size:12px">PDF, Word, TXT, CSV — max 10MB</span></div></div>'
    + '</div>'
    + '<div class="cp-data-pane" id="cpBuildPane-manual" style="display:none">'
    + '<textarea class="cp-textarea" rows="4" placeholder="Type or paste information about your property..."></textarea>'
    + '<button class="btn btn-sm" style="margin-top:8px" onclick="showToast(\'Text added (demo)\', \'success\')">Add</button>'
    + '</div>'
    + '</div>';

  // Section: Conversion URL
  html += '<div class="cp-build-section">'
    + '<h3 class="cp-section-title">4. Conversion URL</h3>'
    + '<p class="cp-build-help">Where should the AI agent send visitors to convert? (e.g. booking page, contact form)</p>'
    + '<div class="cp-sync-row"><input class="search-input" value="' + esc(a.conversionUrl) + '" style="flex:1"><button class="btn btn-sm" onclick="showToast(\'Saved (demo)\', \'success\')">Save</button></div>'
    + '</div>';

  // Section: Embed Code
  html += '<div class="cp-build-section">'
    + '<h3 class="cp-section-title">5. Embed Code</h3>'
    + '<p class="cp-build-help">Add this code to your website to activate the AI agent on your Matterport tour.</p>'
    + '<div class="cp-embed-code"><pre>' + esc(a.embedCode) + '</pre>'
    + '<button class="btn btn-sm btn-outline" onclick="cpCopyEmbed()" style="margin-top:8px">Copy code</button>'
    + '</div>'
    + '</div>';
...
function cpSwitchBuildTab(tab, btn) {
  document.querySelectorAll('.cp-data-pane[id^="cpBuildPane-"]').forEach(function(p) { p.style.display = 'none'; });
  document.querySelectorAll('.cp-data-tab').forEach(function(b) { b.classList.remove('active'); });
  var pane = document.getElementById('cpBuildPane-' + tab);
  if (pane) pane.style.display = 'block';
  if (btn) btn.classList.add('active');
}

function cpUpdateBuildWordBar() {
  var words = DUMMY_AGENT.wordCount;
  var pct = Math.min(100, (words / 2000) * 100);
  var bar = document.getElementById('cpBuildWordBar');
  var label = document.getElementById('cpBuildWordCount');
  if (bar) {
    bar.style.width = pct + '%';
    bar.style.background = pct < 25 ? 'var(--red)' : pct < 60 ? 'var(--orange)' : 'var(--green)';
  }
  if (label) label.textContent = 'Your agent knows ' + words + ' words';
}

function cpCopyEmbed() {
  if (navigator.clipboard) {
    navigator.clipboard.writeText(DUMMY_AGENT.embedCode).then(function() { showToast('Copied!', 'success'); });
  } else {
...
/* ── Tab: Knowledge ── */
function cpRenderKnowledge() {
  var a = DUMMY_AGENT;
  var html = '<div class="cp-section">'
    + '<h3 class="cp-section-title">Data Status</h3>'
    + '<div class="cp-word-bar-wrap"><div class="cp-word-bar" id="cpWordBar"></div></div>'
    + '<div class="cp-word-count" id="cpWordCount"></div>'
    + '</div>';

  html += '<div class="cp-section">'
    + '<div class="cp-data-tabs">'
    + '<button class="cp-data-tab active" onclick="cpSwitchKnowledgeTab(\'sync\', this)">Sync from website</button>'
    + '<button class="cp-data-tab" onclick="cpSwitchKnowledgeTab(\'upload\', this)">Upload file</button>'
    + '<button class="cp-data-tab" onclick="cpSwitchKnowledgeTab(\'manual\', this)">Add manually</button>'
    + '</div>'
    + '<div class="cp-data-pane" id="cpKnPane-sync" style="display:block">'
    + '<div class="cp-sync-row"><input class="search-input" placeholder="https://your-website.com" value="https://harbourviewhotel.com" style="flex:1"><button class="btn btn-sm" onclick="showToast(\'Scraping... (demo)\', \'success\')">Sync</button></div>'
    + '</div>'
    + '<div class="cp-data-pane" id="cpKnPane-upload" style="display:none">'
    + '<div class="cp-dropzone" onclick="showToast(\'File picker (demo)\')"><div class="cp-dropzone-text">Drop a file here or click to upload<br><span style="color:var(--muted);font-size:12px">PDF, Word, TXT, CSV — max 10MB</span></div></div>'
    + '</div>'
    + '<div class="cp-data-pane" id="cpKnPane-manual" style="display:none">'
    + '<textarea class="cp-textarea" rows="4" placeholder="Type or paste information..."></textarea>'
    + '<button class="btn btn-sm" style="margin-top:8px" onclick="showToast(\'Text added (demo)\', \'success\')">Add</button>'
    + '</div>'
    + '</div>';
  ...
  // View & Edit All Data
  html += '<div class="cp-section">'
    + '<h3 class="cp-section-title">View & Edit All Data</h3>'
    + '<textarea class="cp-textarea cp-data-viewer" rows="12">' + esc(a.propertyData) + '</textarea>'
    + '<button class="btn btn-sm" style="margin-top:8px" onclick="showToast(\'Changes saved (demo)\', \'success\')">Save changes</button>'
    + '</div>';

  return html;
}

function cpSwitchKnowledgeTab(tab, btn) {
  document.querySelectorAll('.cp-data-pane[id^="cpKnPane-"]').forEach(function(p) { p.style.display = 'none'; });
  document.querySelectorAll('.cp-data-tab').forEach(function(b) { b.classList.remove('active'); });
  var pane = document.getElementById('cpKnPane-' + tab);
  if (pane) pane.style.display = 'block';
  if (btn) btn.classList.add('active');
}

function cpUpdateWordBar() {
  var words = DUMMY_AGENT.wordCount;
  var pct = Math.min(100, (words / 2000) * 100);
  var bar = document.getElementById('cpWordBar');
  var label = document.getElementById('cpWordCount');
  if (bar) {
    bar.style.width = pct + '%';
    bar.style.background = pct < 25 ? 'var(--red)' : pct < 60 ? 'var(--orange)' : 'var(--green)';
  }
...
function cpRenderSettings() {
  ...
  // Agent settings
  var html = '<div class="cp-section">'
    + '<h3 class="cp-section-title">Agent Settings</h3>'
    + '<div class="cp-settings-grid">'
    + '<div class="cp-setting-field">'
    + '<label class="cp-setting-label">Agent name</label>'
    + '<input class="search-input" value="' + esc(a.agentName) + '" style="width:100%">'
    + '</div>'
    + '<div class="cp-setting-field">'
    + '<label class="cp-setting-label">Language</label>'
    + '<select class="cp-select">';
  languages.forEach(function(l) {
    html += '<option value="' + l.code + '"' + (a.language === l.code ? ' selected' : '') + '>' + l.label + '</option>';
  });
  html += '</select></div>'
    + '<div class="cp-setting-field">'
    + '<label class="cp-setting-label">Brand color</label>'
    + '<input type="color" class="cp-color-input" value="' + esc(a.brandColor) + '">'
    + '</div>'
    + '<div class="cp-setting-field">'
    + '<label class="cp-setting-label">Conversion URL</label>'
    + '<input class="search-input" value="' + esc(a.conversionUrl) + '" style="width:100%">'
    + '</div>'
    + '</div></div>';

  // Demo questions
  html += '<div class="cp-section">'
    + '<h3 class="cp-section-title">Demo Questions</h3>'
    + '<p class="cp-build-help">Suggested questions shown to visitors in the chat widget.</p>';
  for (var i = 0; i < 5; i++) {
    html += '<input class="search-input cp-demo-q" placeholder="Question ' + (i + 1) + '..." value="' + esc(a.demoQuestions[i] || '') + '">';
  }
  html += '<button class="btn btn-sm" style="margin-top:12px" onclick="showToast(\'Settings saved (demo)\', \'success\')">Save settings</button>'
    + '</div>';

  // Usage
  var usedPct = Math.round((a.minutesUsed / a.minutesQuota) * 100);
  var usageColor = usedPct < 60 ? 'var(--green)' : usedPct < 80 ? 'var(--orange)' : 'var(--red)';
  html += '<div class="cp-section">'
    + '<h3 class="cp-section-title">Usage This Month</h3>'
    + '<div class="cp-usage-bar-wrap"><div class="cp-usage-bar" style="width:' + usedPct + '%;background:' + usageColor + '"></div></div>'
    + '<div class="cp-usage-text">' + a.minutesUsed + ' of ' + a.minutesQuota + ' minutes used</div>'
    + '<div class="cp-usage-remaining">' + (a.minutesQuota - a.minutesUsed) + ' minutes remaining &middot; Resets ' + fmtDate(a.quotaResetDate) + '</div>'
    + '</div>';

  // Buy minutes
  html += '<div class="cp-section">'
    + '<h3 class="cp-section-title">Buy Minutes</h3>'
    + '<div class="cp-packages">'
    + cpPackageCard(200, 22, '0.11')
    + cpPackageCard(500, 44, '0.088', true)
    + cpPackageCard(1000, 73, '0.073')
    + '</div>'
    + '<button class="btn" style="margin-top:16px" onclick="showToast(\'Stripe checkout (demo)\', \'success\')">Purchase 500 minutes — $44</button>'
    + '</div>';

  // Billing
  html += '<div class="cp-section">'
    + '<h3 class="cp-section-title">Billing</h3>'
    + '<div class="cp-billing-card">'
    + '<div class="cp-billing-plan">' + esc(a.plan) + ' &middot; ' + a.minutesQuota + ' minutes</div>'
    + '<div class="cp-billing-status"><span class="badge badge-active">' + esc(a.planStatus) + '</span></div>'
    + '<div class="cp-billing-next">Next billing: ' + fmtDate(a.nextBilling) + '</div>'
    + '<button class="btn btn-outline btn-sm" onclick="showToast(\'Contact support for subscription management\')">Manage subscription</button>'
    + '</div></div>';

  return html;
}

function cpPackageCard(minutes, price, perMin, selected) {
  return '<div class="cp-package' + (selected ? ' selected' : '') + '" onclick="cpSelectPackage(this)">'
    + '<div class="cp-package-mins">' + minutes + ' min</div>'
    + '<div class="cp-package-price">$' + price + '</div>'
    + '<div class="cp-package-per">$' + perMin + '/min</div>'
    + '</div>';
}

function cpSelectPackage(el) {
  document.querySelectorAll('.cp-package').forEach(function(p) { p.classList.remove('selected'); });
